...
func (r *CredentialRepo) FindByEmail(ctx context.Context, email string) (*domain.Credential, error) {
	query := `SELECT id, email, password_hash, verified, created_at, is_banned, banned_reason FROM credentials WHERE email = $1`
...
func (r *CredentialRepo) FindByID(ctx context.Context, id uuid.UUID) (*domain.Credential, error) {
	query := `SELECT id, email, password_hash, verified, created_at, is_banned, banned_reason FROM credentials WHERE id = $1`
